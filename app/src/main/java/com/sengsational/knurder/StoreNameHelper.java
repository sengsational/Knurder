...
public class StoreNameHelper {
    ...
    public static void confirmLocationsLoadedInDatabase() {
        ...
        try {
            ...
            if (count < storesNvpArray.length) {  // Load database with HARD CODED if it's not got enough records
                Log.v("sengsational", "store database getting loaded.");
                ...
                for (int i = 0; i < storesNvpArray.length; i++){
                    ...
                    Log.v("sengsational","SNH.confirm " + numberNameNvp[1]);
                    values.put("STORE_NAME", numberNameNvp[1]);
                    values.put("LOCAL_STATES", nameStateNvp[1]);
                    //Log.v("sengsational", " values to string " + values.toString());
                    db.insert("LOCATIONS", null, values);
                }
            } else {
                Log.v("sengsational", "store database was good.");
                // Remove old stores
                try {
                    ...
                    Log.e("sengsational", "Could not delete old store: " + t.getMessage());
                }
            }
        } catch (Exception e) {
            Log.e("sengsational", "Problem in StoreNameHelper.confirmLocationsLoadedInDatabase() " + e.getMessage());
        } finally {
            ...
            while (cursor.moveToNext()){
                ...
                if (localStates != null) {
                    try {
                        ...
                        Log.e("sengsational", "Error parsing [" + localStates + "]");
                    }
                }
            }
        } catch (Exception e) {
            Log.e("sengsational", "Problem in StoreNameHelper.getStoreIdsByStateMapFromDatabase() " + e.getMessage());
        } finally {
        ...
        try {
            ...
            if (names != null && names.length > count) {
                ...
                for (int i = 0; i < names.length; i++){
                    ...
                    Log.v("sengsational", "SNH.reload " + names[i].trim());
            ...
            Log.e("sengsational", "Problem in StoreNameHelper.reloadFromPageAndSaveToDatabase() " + e.getMessage());
        } finally {
            try {
    ...
    public Set<String> getStoreTreeSet() {
        ...
        try {
            ...
            Log.e("sengsational", "Problem in StoreNameHelper.getStoreTreeSet() " + e.getMessage());
        } finally {
            try {ufoHelper.close();} catch (Throwable t){};                                //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<CLOSING DATABASE
        }
        return storeSet;
    ...
    private List<String> getStoreNamesFromDatabase() {
        ...
        try {
            ...
            Log.e("sengsational", "Problem in StoreNameHelper.getStoreNamesFromDatabase() " + e.getMessage());
        } finally {
            try {ufoHelper.close();} catch (Throwable t){};                                //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<CLOSING DATABASE
        }
        return storeNames;
    ...
    public static String getStoreNumberFromName(String storeName){
        ...
        try {
            ...
            if (cursor.moveToFirst()){
                ...
                Log.e("sengsational", "Problem in StoreNameHelper.getStoreNumberFromName().  Did not find  " + storeName  + " in the database.");
            }
        } catch (Exception e) {
            Log.e("sengsational", "Problem in StoreNameHelper.getStoreNumberFromName() " + e.getMessage());
        } finally {
            try {ufoHelper.close();} catch (Throwable t){};                                //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<CLOSING DATABASE
        }
        return storeNumber;
    ...
    public String getStoreNameFromNumber(String storeNumber, SQLiteDatabase db) {
        ...
        try {
            ...
            if (cursor.moveToFirst()){
                ...
                Log.e("sengsational", "Problem in StoreNameHelper.getStoreNameFromNumber().  Did not find  " + storeNumber  + " in the database.");
            }
        } catch (Exception e) {
            Log.e("sengsational", "Problem in StoreNameHelper.getStoreNameFromNumber() " + e.getMessage());
        } finally {
            try {
    ...
    public HashMap<String,String> getStoreMap() {
        ...
        Log.v("sengsational", "Getting store set. Size: " + storeTreeSet.size());
